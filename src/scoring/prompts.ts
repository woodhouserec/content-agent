...
- First summarize what the source material actually says. Then extract one key semantic thesis: the professional point the author could make from that material.
- postTitle/postTitleRu must describe the future post idea, not reuse a generic category template.
- shortDescription/shortDescriptionRu are required. They must be a factual, organic 2-4 sentence summary of the source material itself, grounded in the actual excerpt.
- shortDescription/shortDescriptionRu must help a Russian-speaking reviewer understand what the article is about before deciding whether to create a draft.
- shortDescription/shortDescriptionRu must summarize the source, not pitch the future post.
- Do not repeat the full article title inside shortDescription, recruiterValue, or suggestedAngle.
- Do not start shortDescription with "A post about", "A possible post about", or similar generic wording.
- Do not start shortDescriptionRu with "Пост о", "Возможный пост о", "Статья о том, как <full title>", or similar generic wording.
...
- possibleLinkedInAngle/suggestedAngle/suggestedAngleRu must explain the concrete practitioner angle for this material.
- The angle should answer: "What professional argument could the author make from this article?"
