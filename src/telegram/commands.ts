...
export async function buildStartMessage(): Promise<string> {
  return [
    ...
    "",
    "Я буду помогать выбирать материалы, формировать тезисы и готовить черновики LinkedIn-постов.",
    "Теперь можно работать через кнопки меню: источники, профиль и системные действия.",
    "",
    "Команды:",
    "/help - помощь",
    "/status - статус Worker и D1",
    "/collect - запустить сбор материалов",
    "/score - сгенерировать тезисы из материалов",
    "/topics - показать доступные тезисы",
...
export async function buildHelpMessage(): Promise<string> {
  return [
    ...
    "/score - сгенерировать тезисы из новых материалов",
    "/topics - показать последние доступные тезисы",
    "/profile - показать relevance profile",
    ...
    "",
    "Бот собирает материалы, помогает выбрать тезис будущего поста и создаёт текстовый черновик только по вашему действию."
  ].join("\n");
}
